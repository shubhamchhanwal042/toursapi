            ...
            if ($result != null) {
                ...
                if (isset($_FILES['photos']['name']) && !empty($_FILES['photos']['name'])) {
                    if ($this->upload->do_upload('photos')) {
                        ...
                        // Replace the specific image if specified in the request
                        if (isset($formdata['replace_photo']) && !empty($formdata['replace_photo'])) {
                            $replace_photo = $formdata['replace_photo'];

                            // Find and replace the old photo
                            if (($key = array_search($replace_photo, $existing_photos)) !== false) {
                                $old_file_path = $config['upload_path'] . $existing_photos[$key];
                                if (file_exists($old_file_path)) {
                                    unlink($old_file_path); // Delete the old file
                                }
                                $existing_photos[$key] = $uploaded_file; // Replace with the new file name
                            }
                        } else {
                            // Add the new photo if no specific replacement is mentioned
                            $existing_photos[] = $uploaded_file;
                        }
                    } else {
                        // Handle upload errors
                        $error = $this->upload->display_errors();
                        $this->output->set_status_header(400);
                        $response = array("status" => "error", "message" => "Image upload error: " . $error);
                        $this->output->set_content_type("application/json")->set_output(json_encode($response));
                        return;
                    }
                }
                ...
                if (isset($formdata['delete_photo']) && !empty($formdata['delete_photo'])) {
                    ...
                    if (($key = array_search($delete_photo, $existing_photos)) !== false) {
                        ...
                        unset($existing_photos[$key]); // Remove from the array
                    }
                }
                ...
                if ($update_result) {
                    ...
                    $this->output->set_status_header(500);
                    $response = array("status" => "error", "message" => "Failed to update the banner");
                }
            } else {
                $this->output->set_status_header(404);
                ...
                if (isset($formdata['delete_photo']) && !empty($formdata['delete_photo'])) {
                    ...
                    if (($key = array_search($delete_photo, $existing_photos)) !== false) {
                        ...
                        if (file_exists($file_to_delete)) {
                            ...
                            error_log("File not found: " . $file_to_delete);
                        }
                        ...
                        if ($update_result) {
                            ...
                            $this->output->set_status_header(500);
                            $response = array("status" => "error", "message" => "Failed to update the banner after deletion");
                        }
                    } else {
                        error_log("Image not found in the banner array");
                        $this->output->set_status_header(404);
                        $response = array("status" => "error", "message" => "Image not found in the banner");
                    }
                } else {
                    error_log("No image specified for deletion");
                    $this->output->set_status_header(400);
                    $response = array("status" => "error", "message" => "No image specified for deletion");
                }
            } else {
                error_log("Banner not found in database");
                $this->output->set_status_header(404);
                $response = array("status" => "error", "message" => "Banner not found");
            }
        } else {
